    ...
    public void assembleFields(int offset, byte[] buffer) {
        ...
        if (formBodyLength != 0) {
            try {
                // 实例化Form体
                // 新增系统别判断
                logger.info("Form class:" + "rfm.ta.tagateway.tasbs.txn.model.form." + formHeader.getFormSys().toLowerCase() + "." + formHeader.getFormCode());
                Class clazz = Class.forName("rfm.ta.tagateway.tasbs.txn.model.form." + formHeader.getFormSys().toLowerCase() + "." + formHeader.getFormCode());
