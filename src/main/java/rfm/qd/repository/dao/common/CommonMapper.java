...
public interface CommonMapper {
    ...
    @Select("select nvl(max(company_id)+1,'1000000001') from qd_rs_fdccompany")
    String selectNewCompanyId();

    List<QdRsPayout> selectRsPayoutsByParamPlan(ParamPlan paramPlan);

    @Select("select nvl(max(SERIAL)+1,'3132000000001') from qd_rs_payout")
    String selectMaxPayoutSerial();

    @Select("select nvl(max(SERIAL)+1,'3133000000001') from qd_rs_receive")
    String selectMaxRecvSerial();

    @Select("select nvl(max(LOCAL_SERIAL)+1,'3134000000001') from QD_RS_ACC_DETAIL")
    String selectMaxAccDetailSerial();

    @Select("select nvl(max(SERIAL)+1,'3135000000001') from qd_rs_refund")
    String selectMaxRefundSerial();

    @Select("select sum(t.pl_amount) sumplamount from qd_rs_refund t where t.deleted_flag = '0' group by t.business_no" +
            " having t.business_no = #{business_no}")
    BigDecimal selectSumPlamount(@Param("business_no") String businessNO);

    @Select("select sum(t.pl_amount) sumplamount from qd_rs_refund t where t.deleted_flag = '0' and pk_id <> #{pkid} group by t.business_no")
    BigDecimal selectSumPlamountExceptPkid(@Param("pkid") String pkid);

    @Select(" select t.account_code, t.account_name, sum(t.trade_amt) as trade_amt , t.trade_date from QD_RS_ACC_DETAIL t " +
    ...
    @Select("select seq_no from qd_rs_sys_ctl where SYS_NO = '1' for update")
    int selectSysSeq();

    @Update("update qd_rs_sys_ctl set seq_no = #{seq}  where SYS_NO = '1'")
    public int updateSysSeq(@Param("seq") int seq);

    @Update("update qd_rs_sys_ctl set seq_no = #{seq}, sys_date = #{date}  where SYS_NO = '1'")
    public int updateSeqAndSysDate(@Param("seq") int seq, @Param("date") String date);

    @Select("select min(t.txn_serial_no) from QD_CBS_ACC_TXN t" +
    ...
    @Select("select acc.account_code as accountNo,acc.account_name as accountName, " +
            ...
            "            from QD_RS_ACCOUNT acc  " +
            "            left join " +
            "            (select * from QD_CBS_ACC_TXN where txn_date = #{date}" +
            "             and send_flag = '0') t  " +
    ...
    @Update("update QD_CBS_ACC_TXN t set t.send_flag = '1' where t.txn_date = #{date} and t.send_flag = '0'")
    public int updateCbsActtxnsSent(@Param("date") String date);

    @Update("update QD_CBS_ACC_TXN t set t.send_flag = '0' where t.txn_date = #{date} and t.send_flag = '1'")
